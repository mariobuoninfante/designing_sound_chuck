...
private class Clip extends Chugen
{
    -1 => float min;    // range min
    1  => float max;    // range max
    
    function void set(float mi, float ma)
    {
        mi => min;
        ma => max; 
    }
...
SinOsc osc => Gain g => Clip clip => blackhole;
osc.freq(640);
g.gain(1000000000);
clip.set(-0.9, 0.9);
...
plot1.record(osc, (1000./640.)*2::ms);      // plot original signal
plot2.record(clip, (1000./640.)*2::ms);      // plot square
1.1::second => now;
