// 13.6 min and max of a signal

private class Min extends Chugen
{
    0 => float min;
    
    fun float tick(float in)
    {
        return Math.min(in, min); 
    }
}

private class Max extends Chugen
{
    0 => float max;
    
    fun float tick(float in)
    {
        return Math.max(in, max); 
    }
}
...
SinOsc osc => Min min => blackhole;
osc => Max max => blackhole;
osc.freq(640);

samp => now;

plot1.record(min, (1000./640.)*2::ms);      // plot Min
plot2.record(max, (1000./640.)*2::ms);      // plot max
1.1::second => now;
